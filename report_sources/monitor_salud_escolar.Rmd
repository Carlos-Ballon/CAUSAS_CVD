...
data_shiny <- data_shiny |>
  dplyr::select(
    # ID
    record_id, sexo_factor, edad_anios, nivel_educativo_madre, nivel_economico,
    seguro_de_salud_adolescente,
    
    # Salud mental
    PHQ_9,
    depresion, GAD_7, ansiedad, afhc_score, dinamometria, categoria_afhc, 
    paqc_final_score, paqc_dx,
    
    # Alergias
    alergia_pteronyssinus, alergia_farinae, alergia_destructor,
    alergia_tropicalis, alergia_perro, alergia_germanica, asma, 
    rinitis_alergica, feno,
    ...
    PAS, PAD, pulso, metabolic_health, TG_LDL_dx, bp_category, crit_tg, crit_hdl,
    riesgo_aterogenico, dyslipidemia_algorithm, crit_glucosa, TG_HDL_ratio, TyG, 
    TyG_IR, sindrome_metabolico, VAI_sindrome_metabolico,
    
    # Antropometria
    peso_kg, talla_m, imc, zbfa_dx, zhfa_dx, grasa_ip, VAI, indice_cintura_cadera, 
    indice_cintura_talla, WHR_cardio, dinamometria, absi, 
...
# boton_hd("seccion_riesgo_cardiometabolico", "Perfil lipidico", "info")
...
### Salud cardiovascular (NHLBI/CDC/AAP)
...
### Riesgo cardiometabólico <br> (Dislipidemia aterogénica)

```{r}
div(id = "seccion_riesgo_cardiometabolico", renderValueBox({
  d <- datos_alumno()
  color_diag <- case_when(
    d$riesgo_aterogenico == "Riesgo" ~ "danger",
    d$riesgo_aterogenico == "Normal" ~ "success",
    TRUE ~ "primary"
  )
  valueBox(d$riesgo_aterogenico, icon = "fa-flask", color = color_diag)
}))
```
...
div(id = "seccion_clinica", tags$style(
  ...
  data.frame(
    ...
    "Referencia" = c(
      ...
      "< 100", # Glucosa plasmática en ayunas (mg/dL)
      "< 90", # Triglicéridos (mg/dL)
      "< 170", # Colesterol Total (mg/dL)
      "< 40", # Colesterol HDL (mg/dL)
      "< 110", # Colesterol LDL (mg/dL)
...
div(id = "seccion_espirometria", fluidRow(column(
  ...
  wellPanel(
    ...
    tags$ul(
      ...
      tags$li(strong("HDL-c:"), " Lipoproteínas de Alta Densidad"),
      tags$li(strong("LDL-c:"), " Lipoproteína de baja densidad"),
      ...
      tags$li(strong("PAQ-A:")," Physical Activity Questionnaire for Adolescents"),
      tags$li(strong("ISAAC:"), " International Study of Asthma and Allergies in Childhood"),
    ...
    tags$p(
      style = "font-size: 12px; color: #d9534f; font-style: italic;",
    ...
    tags$p(
      ...
      "Se recomienda que, ante cualquier indicador de salud cardiometabólica que supere los valores de referencia establecidos, el adolescente sea derivado para una evaluación especializada por pediatría, nutrición, endocrinología o médico general. Como medida inicial, se sugiere la implementar pautas nutricionales que promuevan la salud cardiovascular mediante un aporte de grasas totales de entre el 25-30% de las calorías diarias, con una aporte grasas saturadas entre 8-10% (NHLBI)."
    ),
  )
)))
```
