...
data_clear <- data_clear |>
  mutate(   
    ...
    across(
      ...
      ~ case_match(.x, "Negativo" ~ "No", "Positivo" ~ "Yes")
    ),
    
    across(c(asma, rinitis_alergica), ~ fct_recode(.x, "Yes" = "Sí")),
    
    # --- MÓDULO ASMA ---
    # Prevalencia de vida: ¿Alguna vez pitos?
    asma_vida = ifelse(isaac_asma_21 == "Sí", 1, 0),
    
    # Prevalencia actual: Pitos en los últimos 12 meses (Indicador clave)
    asma_actual = ifelse(isaac_asma_22 == "Sí", 1, 0),
    ...
    asma_severa = case_when(
      asma_actual == 0 ~ 0,
      asma_actual == 1 & (isaac_asma_23 %in% c("4 a 12", "Más de 12") | 
    ...
    # --- MÓDULO RINITIS ---
    rinitis_vida = ifelse(isaac_rinitis_29 == "Sí", 1, 0),
    rinitis_actual = ifelse(isaac_rinitis_30 == "Sí", 1, 0),
    
    # Rinoconjuntivitis (Síntomas nasales + ojos llorosos/picor)
    rinoconjuntivitis = ifelse(rinitis_actual == 1 & isaac_rinitis_33 == "Sí", 1, 0),

    # --- MÓDULO DERMATITIS ---
    eczema_vida = ifelse(isaac_dermatitis_35 == "Sí", 1, 0),
    eczema_actual = ifelse(isaac_dermatitis_36 == "Sí", 1, 0)
  )
```
...
tbl_1 <- data_clear |>
  dplyr::select(
    pariente_del_infante, edad_pariente, estado_civil_pariente,
    nivel_educativo_pariente, nivel_economico, seguro_de_salud_adolescente, 
    ano_residencia, orden_del_hijo, cluster) |>
  tbl_summary(
    by = cluster,
    digits = list(
...
tbl_2 <- data_clear |>
  dplyr::select(
    ...
    inf_respirat_recurrentes, hospita_previas, vacunas, cluster) |>
  tbl_summary(
    by = cluster,
    digits = list(
      all_continuous() ~ c(2, 2),
    ...
    label = list(
      ...
      enf_macrosomia ~ "Macrosomía (peso mayor de 4 Kg al nacer)",
...
tbl_3 <- data_clear |>
  dplyr::select(
    ...
    ant_cancer, ant_obesidad, ant_familiar_otros, cluster) |>
  tbl_summary(
    by = cluster,
    digits = list(
      all_continuous() ~ c(2, 2),
      all_categorical()~ c(1, 1)),
    label = list(
      ant_familiar_pulmon ~ "Antecedente familiar de enfermedad pulmonar",
      ant_diabetes2 ~ "Antecedente familiar de Diabetes Mellitus 2",
      ant_hta ~ "Antecedente familiar de HTA",
      ant_fenf_cardiov ~ "Antecedente familiar de CVD",
      ant_dislipidemias ~ "Antecedente familiar de dislipidemias",
      ant_cancer ~ "Antecedente familiar de cáncer",
      ant_obesidad ~ "Antecedente familiar de obesidad",
      ant_familiar_otros ~ "Otro antecedentes familiar"
    ),
    missing = "no"
  ) |>
  add_overall() |>
...
tbl_4 <- data_clear |>
  dplyr::select(ansiedad_1, ansiedad_2, depresion_1, depresion_2, PHQ_9,
    ...
    categoria_afhc, paqc_final_score, paqc_dx, cluster) |>
  tbl_summary(
    by = cluster,
    digits = list(
      all_continuous() ~ c(2, 2),
...
```{r}
# tbl_5 <- data_clear |>
#   dplyr::select(exposoma_1_1a:exposoma_2_6b, cluster) |>
#   tbl_summary(
#     by = cluster,
...
## Tabla 6. Alegias, asma y rinitis
...
tbl_7 <- data_clear |>
  select(
    ...
    eczema_actual, cluster) |>
  tbl_summary(
    by = cluster, # Si quieres comparar por tus grupos de salud metabólica
    label = list(
...
tbl_8 <- data_clear |>
  dplyr::select(
    ...
    TyG, VAI, metabolic_health, TyG, cluster) |>
  tbl_summary(
    by = cluster,
...
tbl_10 <- data_clear |>
  dplyr::select(
    ...
    TyG_WC, TyG_BMI, TyG_WHtR, TG_HDL, cluster) |>
  tbl_summary(
    by = cluster,
    digits = list(all_continuous() ~ c(2, 2), all_categorical() ~ c(1, 1)),
    statistic = list(all_categorical() ~"{n} ({p}%)"),
...
tbl_11 <- data_clear |>
  dplyr::select(
    grasa_boileau, grasa_slaughet, grasa_deurenberg, grasa_ict, grasa_ip, cluster) |>
  tbl_summary(
    by = cluster,
    digits = list(all_continuous() ~ c(2, 2), all_categorical() ~ c(1, 1)),
    statistic = list(all_categorical() ~"{n} ({p}%)"),
...
# Stack tables
tables = tbl_stack(
  list(tbl_1, tbl_2, tbl_3, tbl_4, tbl_6, tbl_7, tbl_8, tbl_9, tbl_10, tbl_11),
  group_header = c(
