...
tbl_8 <- data_clear |>
  ...
  bold_labels() |>
  bold_p(t = 0.05) |>
  # Simple star for continuous (Kruskal-Wallis)
  modify_fmt_fun(p.value ~ function(x)
    ifelse(is.na(x), "", paste0(style_pvalue(x, digits = 3), "*")),
    rows = var_type == "continuous") |>
  # Double star for categorical (Chi-square and Fisher)
  modify_fmt_fun(
    p.value ~ function(x)
      ifelse(is.na(x), "", paste0(style_pvalue(x, digits = 3), "**")),
...
tabla_1 <- data_clear |>
  dplyr::select(
    # Perfil Metabólico
    metabolic_health, sindrome_metabolico, VAI_sindrome_metabolico,
    
    # Hemodinámica
    PAS, PAD, bp_category, pulso, WHR_cardio,
    
    # Bioquímica
    glucosa, colesterol_total, trigliceridos, hdl_colesterol,
    
    # Índices
    TyG, TyG_IR, TG_HDL_dx, VAI, riesgo_aterogenico,
  ...
  tbl_summary(
    ...
    label = list(
      ...
      TyG ~ "Índice triglicéridos-glucosa (TyG)",
      VAI ~ "Índice de adiposidad visceral (VAI)",
      VAI_sindrome_metabolico ~ "Síndrome metabólico basado en VAI",
      sindrome_metabolico ~ "Síndrome metabólico",
      metabolic_health ~ "Fenotipo metabólico",
      TyG_IR ~ "Insulinorresistencia basada en TyG",
      TG_HDL_dx	~ "Perfil lipídico",
      WHR_cardio ~ "Riesgo cardiovascular basado en ICC",
  ...
  modify_header(
    ...
    stat_3 = "**Jesus y María - La Molina**") |>
  modify_footnote(
    starts_with("p.value") ~ 
    "Prueba de Kruskal-Wallis (*); Prueba exacta de Fisher con simulación de Montecarlo (**); Prueba exacta de Fisher (†); V de Cramer (†); Magnitud del efecto Eta-cuadrado (‡)"
  ) |>
  as_flex_table() |>
  theme_vanilla() |>
