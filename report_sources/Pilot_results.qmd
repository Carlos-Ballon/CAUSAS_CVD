...
csl: elsevier-vancouver-no-et-al.csl
---
...
grateful::cite_packages(
  ...
  out.format = "docx",
  out.dir = ".",
  citation.style = "elsevier-vancouver-no-et-al")
```
...
data_clear <- data |>
  mutate(
    ...
    seguro_de_salud_adolescente = case_match(
      seguro_de_salud_adolescente, 
      "EsSalud" ~ "EsSalud",
      "SIS" ~ "Seguro Integral de Salud (MINSA)"),
    ...
    LDL_friedewald = colesterol_total - hdl_colesterol - (trigliceridos/5),
    ...
    TyG_IR = if_else(
      TyG >= 8.19, "Resistencia a la insulina", "Sin riesgo"),
    ...
    colesterol_total_dx = case_when(
      colesterol_total >= 200 ~ "Alto",
      colesterol_total >= 170 ~ "Límite",
      colesterol_total < 170 ~ "Aceptable"
    ),
    
    LDL_dx = case_when(
      LDL_friedewald >= 130 ~ "Alto",
      LDL_friedewald >= 110 ~ "Límite",
      LDL_friedewald < 110 ~ "Aceptable"
    ), 
    
    HDL_dx = case_when(
      hdl_colesterol < 40 ~ "Bajo", # Riesgo
      hdl_colesterol <= 45 ~ "Límite",
      hdl_colesterol > 45 ~ "Aceptable"
    ),

    TG_dx = case_when(
      trigliceridos >= 130 ~ "Alto", 
      trigliceridos >= 90 ~ "Límite",
      trigliceridos < 90 ~ "Aceptable"),
    
    # Perfil lipídico
    TG_LDL_dx = case_when(
      LDL_friedewald >= 130 | colesterol_total >= 200 ~ "Hipercolesterolemia", # Alto
      LDL_friedewald >= 110 | colesterol_total >= 170 ~ "Hipercolesterolemia", # Límite
      TRUE ~ "Aceptable"
    ), 
    
    dyslipidemia_algorithm = case_when(
      LDL_friedewald >= 130 | trigliceridos >= 130 | hdl_colesterol < 40 ~ "Dislipidemia",
      TRUE ~ "Saludable" # Solo si LDL < 110 AND TG < 90 AND HDL > 45
    ),
    
    perfil_aterogenico_dx = case_when( # Gemini AI
      trigliceridos >= 130 & hdl_colesterol < 40 ~ "Dislipidemia aterogénica",
      trigliceridos >= 130 | hdl_colesterol < 40 ~ "Riesgo aislado",
      (trigliceridos >= 90 & trigliceridos < 130) |
        (hdl_colesterol >= 40 & hdl_colesterol <= 45) ~ "Límite",
      TRUE ~ "Aceptable"
    ),
    ...
    sexo_deurenberg = if_else(sexo == 1, 1, 0),
    ...
    # RFM
    sexo_rfmp = if_else(sexo == 1, 0, 1),
    RFMp = 74 - (22 * talla_cm/perimetro_cintura) + (5 * sexo_rfmp),
  ...
  mutate(
    zbfa_dx = case_when(
      zbfa > 2 ~ "Exceso de peso", # Obesidad
      zbfa > 1 ~ "Exceso de peso", # Sobrepeso
      zbfa < -2 ~ "Delgadez",
      TRUE ~ "Normal"
    ) |>
    ...
    zhfa_dx = case_when(
      ...
      ff_label("Talla para la edad (z-score)"),
...
data_clear <- data_clear |>
  mutate(
    ...
    p95_bp = if_else(sbp_p >= 95 | dbp_p >= 95, 1, 0), 
    ...
    bp_category = case_when(
      ...
      edad_anios >= 13 ~ case_when(
        ...
        PAS >= 120 & PAD < 80 ~ "Elevada", # Presión Elevada
...
data_clear <- data_clear |>
  mutate(   
    ...
    nivel_educativo_madre = case_when(
      nivel_educativo_pariente %in% c("Inicial", "Primaria") & 
        pariente_del_infante == "Madre" ~ "Primaria",
      nivel_educativo_pariente == "Secundaria" & 
        pariente_del_infante == "Madre" ~ "Secundaria",
      nivel_educativo_pariente %in% c("Técnica", "Universitaria") & 
        pariente_del_infante == "Madre" ~ "Superior",
      TRUE ~ NA_character_) |>
    ...
    nivel = case_match(
      colegio_id,
      "Colegio Jorge Basadre - VMT" ~ "Nivel 2",
      "Colegio 0028 Jesus y María - La Molina" ~ "Nivel 1",
      "Colegio Santísima Virgen de Chapi - Pachacamac" ~ "Nivel 3"
    ) |>
      fct_relevel("Nivel 1", "Nivel 2", "Nivel 3")
  )
...
tbl_1 <- data_clear |>
  dplyr::select(
    ...
    nivel_educativo_madre, nivel_economico, seguro_de_salud_adolescente, 
...
tbl_4 <- data_clear |>
  ...
  tbl_summary(
    ...
    label = list(
      ...
      paqc_final_score ~ "Puntaje del PAQ-C",
...
tbl_8 <- data_clear |>
  dplyr::select(
    glucosa, trigliceridos, hdl_colesterol, LDL_friedewald, colesterol_total,
    TG_LDL_dx, TG_HDL_ratio, riesgo_aterogenico, dyslipidemia_algorithm, 
    PAS, PAD, pulso, TyG, VAI, VAI_sindrome_metabolico, sindrome_metabolico, 
    metabolic_health, bp_category, TyG_IR, colegio_id) |>
  tbl_summary(
    by = colegio_id,
    ...
    label = list(
      ...
      TyG_IR ~ "Resistencia a la insulina (TyG)",
...
tbl_10 <- data_clear |>
  ...
  tbl_summary(
    ...
    label = list(
      ...
      indice_cintura_talla ~ "Índice cintura-talla (ICT)",
...
tbl_11 <- data_clear |>
  dplyr::select(
    grasa_boileau, grasa_slaughet, grasa_deurenberg, grasa_ict, grasa_ip, RFMp, 
    colegio_id) |>
  tbl_summary(
    by = colegio_id,
    digits = list(all_continuous() ~ c(2, 2), all_categorical() ~ c(1, 1)),
...
El tamaño del efecto se suele interpretar así:

Eta-cuadrado ($\eta^2$): ~0.01 (pequeño), ~0.06 (moderado), >0.14 (grande). 
V de Cramer (3x4): Al ser una tabla de este tamaño, un valor >0.15 suele considerarse un efecto moderado y >0.25 un efecto fuerte.
...
tabla_1 <- data_clear |>
  dplyr::select(
    ...
    riesgo_aterogenico, sindrome_metabolico, metabolic_health, nivel_economico, nivel_educativo_madre, seguro_de_salud_adolescente, RFMp, zbfa
  ) |>
  tbl_summary(
    label = list(
      sexo_factor	~ "Sexo",
      ...
      bp_category ~ "Riesgo cardiovascular (NHLBI/CDC/AAP)",
      sindrome_metabolico ~ "Síndrome metabólico (IDF)",
      metabolic_health ~ "Fenotipo metabólico (WGOC)",
      riesgo_aterogenico ~ "Riesgo cardiometabólico (Dislipidemia aterogénica)"
    ),
    missing = "no",
  ...
  modify_abbreviation(
    "IMC: Índice de Masa Corporal; OMS: Organización Mundial de la Salud; IDF: International Diabetes Federation; WGOC: Working Group on Obesity in China; NHLBI: National Heart Lung and Blood Institute; CDC: Centers for Disease Control and Prevention; AAP: American Academy of Pediatrics") |>
  as_flex_table() |>
  theme_booktabs() |>
...
tabla_2 <- data_clear |>
  dplyr::select(
    nivel_economico, nivel_educativo_madre, seguro_de_salud_adolescente, 
    sexo_factor, edad_anios, peso_kg, talla_m, zbfa_dx, zbfa, grasa_ip, RFMp,
    colegio_id) |>
  tbl_summary(
    by = colegio_id,
    percent = "row",
  ...
  modify_caption("Tabla 2. Características sociodemográficas y estado nutricional según institución educativa.") |>
  modify_abbreviation(
    "IMC: Índice de Masa Corporal; RFMp: Relative Fat Mass pediatric; OMS: Organización Mundial de la Salud; MINSA: Ministerio de Salud del Perú") |>
  modify_footnote(
    starts_with("p.value") ~ 
...
tabla_3 <- data_clear |>
  dplyr::select(
    ...
    # Bioquímica
    glucosa, colesterol_total, trigliceridos, hdl_colesterol, LDL_friedewald, 
    TG_LDL_dx, dyslipidemia_algorithm,
  ...
  tbl_summary(
    ...
    label = list(
      ...
      hdl_colesterol ~ "HDL-c (mg/dL)",
      LDL_friedewald ~ "LDL-c (mg/dL)†††",
      TG_LDL_dx	~ "Perfil lipídico (NHLBI)†††",
      PAS ~ "Presión arterial sistólica (mmHg)",
      PAD ~ "Presión arterial diastólica (mmHg)",
      pulso ~ "Pulso cardíaco (latidos por minuto)",
      bp_category ~ "Riesgo cardiovascular (NHLBI/CDC/AAP)",
  ...
  modify_header(
    ...
    p.value = "**Valor p**",
    add_stat_1 = "**Tamaño del efecto**") |>
  modify_caption("Tabla 3. Comparación de indicadores de salud cardiometabólica entre las instituciones educativas.") |>
  modify_abbreviation(
    "HDL-c: High density lipoprotein; LDL-c: Low density lipoprotein; TyG: Índice triglicéridos-glucosa; VAI: Índice de adiposidad visceral; ICC: Índice Cintura-Cadera; ICT: Índice Cintura-Talla; IDF: International Diabetes Federation; WGOC: Working Group on Obesity in China; NHLBI: National Heart Lung and Blood Institute; CDC: Centers for Disease Control and Prevention; AAP: American Academy of Pediatrics") |>
  modify_footnote(
    starts_with("p.value") ~ 
    "Prueba de Kruskal-Wallis (*); Prueba exacta de Fisher con simulación de Montecarlo (**)",
    add_stat_1 ~ "V de Cramer (†); Magnitud del efecto Eta-cuadrado (‡); el colesterol LDL fue estimado mediante la fórmula de Friedewald"
  ) |>
  as_flex_table() |>
  theme_booktabs() |>
  align(align = "center", part = "header") |>
...
tabla_4 <- data_clear |>
  ...
  modify_header(
    ...
    p.value = "**Valor p**",
    add_stat_1 = "**Tamaño del efecto**") |>
  modify_caption("Tabla 4. Comparación de indicadores de salud mental y estilos de vida saludables entre las instituciones educativas.") |>
  modify_abbreviation(
    "PHQ-9: Patient Health Questionnaire-9; GAD-7: Generalized Anxiety Disorder-7; AFHC: Adolescent Food Habits Checklist; PAQ-C: Physical Activity Questionnaire for Older Children") |>
  modify_footnote(
    starts_with("p.value") ~ 
    "Prueba de Kruskal-Wallis (*); Prueba exacta de Fisher con simulación de Montecarlo (**)",
    add_stat_1 ~ "V de Cramer (†); Magnitud del efecto Eta-cuadrado (‡)"
...
```{r}
data_causas <- data_clear |>
  dplyr::select(
    edad_anios, sexo_factor, nivel_economico, nivel_educativo_madre, 
    seguro_de_salud_adolescente, PAS, PAD, glucosa, colesterol_total, 
    trigliceridos, hdl_colesterol, LDL_friedewald, VAI, TyG, 
    alergia_pteronyssinus, alergia_destructor, alergia_germanica, asma, 
    peso_kg, talla_m, zbfa, zbfa_dx, grasa_ip, RFMp, indice_cintura_cadera,
    indice_cintura_talla, PHQ_9, GAD_7, afhc_score, paqc_final_score, nivel
  )

# Variables para modelos lineales (gaussian)
vars_linear <- c(
  "PAS", "PAD", "glucosa", "colesterol_total", "trigliceridos", 
  "hdl_colesterol", "LDL_friedewald", "VAI", "TyG", "peso_kg", 
  "talla_m", "zbfa", "grasa_ip", "RFMp", "indice_cintura_cadera", 
  "indice_cintura_talla", "PHQ_9", "GAD_7", "afhc_score", "paqc_final_score"
)

# Variables para modelos log-binomiales (requieren recodificación a 0/1)
vars_logbin <- c(
  "alergia_pteronyssinus", "alergia_destructor", 
  "alergia_germanica", "asma", "zbfa_dx"
)
...
run_my_models <- function(var_name, data) {
  
  # Determinar la etiqueta a mostrar
  var_label <- ifelse(
    var_name %in% names(nombres_bonitos), 
    nombres_bonitos[var_name], 
    var_name)
  
  if (var_name %in% vars_linear) {
    # --- Modelo Lineal ---
    formula_ml <- as.formula(paste(var_name, "~ nivel"))
    model <- glm(formula_ml, data = data, family = gaussian(link = "identity")) |> 
      tbl_regression(
        exponentiate = FALSE,
        label = list(nivel ~ var_label))
    
  } else {
    # --- Modelo Log-Binomial ---
    # Recodificamos dinámicamente según tus criterios
    data_mod <- data |> 
      mutate(!!sym(var_name) := case_when(
        .data[[var_name]] %in% c("Sí", "Exceso de peso") ~ 1L,
        is.na(.data[[var_name]]) ~ NA_integer_,
        TRUE ~ 0L
      ))
    
    formula_lb <- as.formula(paste(var_name, "~ nivel"))
    model <- logbin(formula_lb, data = data_mod, method = "em") |> 
      tbl_regression(
        exponentiate = TRUE,
        label = list(nivel ~ var_label))
  }
...
# Unimos todas las variables en una sola lista para iterar
all_vars <- c(vars_linear, vars_logbin)

# Generamos la lista de tablas y las apilamos
tabla_final <- all_vars |> 
  map(~ run_my_models(.x, data_causas)) |> 
  tbl_stack() |> 
  bold_labels()
...
delta <- 2.21         # Diferencia esperada en PAS (mmHg)
sd_outcome <- 8.73    # Desviación estándar estimada
efecto_d <- delta / sd_outcome  # d de Cohen calculado (~0.253)

m <- 32               # Meta de niños por colegio (seguimiento completo)
icc <- 0.02           # Correlación intraclase
alfa <- 0.05          # Nivel de significancia
poder <- 0.80         # Potencia (80%)

# --- 2. CÁLCULO INDIVIDUAL (Base) ---
# Calculamos el n por grupo asumiendo independencia
calc_n <- pwr.t.test(d = efecto_d, 
                     sig.level = alfa, 
                     power = poder, 
                     type = "two.sample", 
...
n_indiv_por_grupo <- ceiling(calc_n$n) 

# Total de niños sin considerar colegios (Grupo A + Grupo B)
n_indiv_total <- n_indiv_por_grupo * 2

# --- 3. AJUSTE POR CONGLOMERADOS (Cluster) ---
# Factor de inflación de la varianza (Design Effect)
de <- 1 + icc * (m - 1)

# Número total de niños ajustado por el efecto de diseño
n_ajustado <- ceiling(n_indiv_total * de)

# --- 4. CÁLCULO DE COLEGIOS Y RECLUTAMIENTO ---
# Número de colegios (k) necesarios
k_total <- ceiling(n_ajustado / m)

# Ajuste por pérdidas del 10% para saber cuántos reclutar al inicio
m_reclutamiento <- ceiling(m / 0.90) 
n_final_reclutamiento <- k_total * m_reclutamiento

# --- 5. RESULTADOS ---
cat("--- RESULTADOS DEL CÁLCULO DE MUESTRA ---\n")
cat("Tamaño del Efecto (d):", round(efecto_d, 3), "\n")
cat("N por grupo (sin ajuste):", n_indiv_por_grupo, "\n")
cat("Efecto de Diseño (DE):", round(de, 2), "\n")
cat("Niños necesarios (total ajustado - seguimiento):", n_ajustado, "\n")
cat("Colegios necesarios (k):", k_total, "\n")
cat("Niños a reclutar por colegio (con pérdidas):", m_reclutamiento, "\n")
cat("TOTAL NIÑOS A RECLUTAR:", n_final_reclutamiento, "\n")
```
