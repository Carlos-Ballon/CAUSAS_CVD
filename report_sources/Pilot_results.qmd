...
data_clear <- data |>
  mutate(
    ...
    across(
      c(TyG_WC, TyG_BMI, TyG_WHtR, TG_HDL),
      ~ cut(
        .,
  ...
  mutate(
    ...
    bp_category = case_when(
      ...
      edad_anios >= 13 ~ case_when(
        PAS >= 140 | PAD >= 90 ~ "Hipertensión Etapa 2",
        PAS >= 130 | PAD >= 80 ~ "Hipertensión Etapa 1",
      ...
      edad_anios < 13 ~ case_when(
        # Etapa 2: >= (P95 + 12 mmHg)
        PAS >= (p95_sbp_mmHg + 12) | PAD >= (p95_dbp_mmHg + 12) ~ "Hipertensión Etapa 2",
        
        # Etapa 1: >= P95
        sbp_p >= 95 | dbp_p >= 95 ~ "Hipertensión Etapa 1",
        
        # Elevada: >= P90 O >= 120/80
        sbp_p >= 90 | dbp_p >= 90 ~ "Elevada",
        
        # Normal: Todo lo que no entró en las anteriores y tiene datos!is.na(PAS) &
        sbp_p < 90 & dbp_p < 90 ~ "Elevada",
    ...
    bp_category = factor(bp_category) |>
      fct_relevel("Normal", "Elevada", "Hipertensión Etapa 1", "Hipertensión Etapa 2") |>
      ff_label("Diagnóstico de hipertensión arterial en adolescentes"),
...
tbl_8 <- data_clear |>
  dplyr::select(
    ...
    TyG, VAI, metabolic_health, bp_category, colegio_id) |>
  tbl_summary(
    by = colegio_id,
...
tbl_10 <- data_clear |>
  dplyr::select(
    ...
    TyG_WC, TyG_BMI, TyG_WHtR, TG_HDL, colegio_id) |>
  tbl_summary(
    by = colegio_id,
    digits = list(all_continuous() ~ c(2, 2), all_categorical() ~ c(1, 1)),
    statistic = list(all_categorical() ~"{n} ({p}%)"),
