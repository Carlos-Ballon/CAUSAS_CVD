...
tabla_1 <- data_clear |>
  dplyr::select(
    ...
    riesgo_aterogenico, sindrome_metabolico, metabolic_health
  ) |>
  tbl_summary(
    label = list(
      sexo_factor	~ "Sexo",
...
tabla_2 <- data_clear |>
  dplyr::select(
    ...
    sexo_factor, edad_anios, peso_kg, talla_m, zbfa_dx, grasa_ip, 
    colegio_id) |>
  tbl_summary(
    by = colegio_id,
    percent = "row",
  ...
  modify_abbreviation(
    "IMC: Índice de Masa Corporal; OMS: Organización Mundial de la Salud; MINSA: Ministerio de Salud del Perú") |>
  modify_footnote(
    starts_with("p.value") ~ 
