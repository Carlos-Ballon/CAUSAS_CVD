...
data_clear <- data |>
  mutate(
    ...
    TyG_IR = if_else(
      TyG >= 8.19, "Riesgo de insulinorresistencia", "Sin riesgo de insulinorresistencia"),
    ...
    WHR_cardio = case_when(
      ...
      sexo == 1 & indice_cintura_cadera >= 90 ~ "Riesgo cardiovascular", # Hombres
      sexo == 2 & indice_cintura_cadera >= 80 ~ "Riesgo cardiovascular", # Mujeres
    ...
    VAI_sindrome_metabolico = case_when(
      ...
      # 2. Definimos ALTO RIESGO (Superan el corte)
      sexo == 1 & VAI >= 1.685 ~ "Alto Riesgo (MetS)", # Hombres
      sexo == 2 & VAI >= 1.875 ~ "Alto Riesgo (MetS)", # Mujeres
  ...
  mutate(
    ...
    bp_category = case_when(
      ...
      edad_anios < 13 ~ case_when(
        ...
        sbp_p >= 95 | dbp_p >= 95 | 
          (PAS >= 130 & PAS < 140) | (PAD >= 80 & PAD < 90) ~ "Hipertensión Etapa 1",
...
data_clear <- data_clear |>
  mutate(   
    ...
    enfermedad_pulmonar = case_when(
      enf_pulmonares_rinitis_alergica == "Sí" ~ "Rinitis alérgica",
      enf_pulmonares_asma == "Sí" ~ "Asma",
      enf_pulmonares_bronquitis_cronica == "Sí" ~ "Bronquitis crónica",
      enf_pulmonares_ninguna == "Sí" ~ "Ninguna",
      TRUE ~ NA_character_) |>
      fct_relevel(
...
tbl_10 <- data_clear |>
  dplyr::select(
    ...
    TyG_WC, TyG_BMI, TyG_WHtR, TG_HDL_ratio, colegio_id) |>
  tbl_summary(
    by = colegio_id,
    digits = list(all_continuous() ~ c(2, 2), all_categorical() ~ c(1, 1)),
    statistic = list(all_categorical() ~"{n} ({p}%)"),
